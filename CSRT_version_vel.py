...
import pandas as pd
from la_velocita_function import la_velocita
...
while True:
    ...
    # angle, speed, velocity = la_velocita(deg,time,v_time,vec_time,coordinates,v_coo,vec_coo,vec_vel,velocity,vec_speed,vec_angle)
    angle, speed, velocity = la_velocita(deg,time,v_coo,vec_vel,velocity,vec_speed,vec_angle)
...
length=np.nonzero(length)     # indices where there are ones
...
np.savetxt(results_folder+'output_data.csv', data, header=head, delimiter=",")
np.savetxt(results_folder+'failure.csv', failure, delimiter=',')
